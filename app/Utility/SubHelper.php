...
function genMultiServersJson($servers)
{
    ...
    file_put_contents(storage_path('app/public/storage/multiConfLeastPing.json'), $jsonContent);

    $jsonContent = json_encode($JsonConfigs, JSON_PRETTY_PRINT);

    //  Write JSON content to file
    
    file_put_contents(storage_path('app/public/storage/multiConf.json'), $jsonContent);
...
function genMultiServersLink($servers)
{
    ...
    file_put_contents(storage_path('app/public/storage/links.sub'), $serversLinks);
}
